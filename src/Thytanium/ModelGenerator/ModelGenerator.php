...
class ModelGenerator
{
    ...
    {
        $matches = $fields = [];
        preg_match_all("#\\$\\w+\\-\\>(string|(unsigned)?(tiny|small|medium|big|long)?(text|integer)|enum|binary|boolean|char|date|datetime|decimal|double|float|time)\\s*\\(\\s*[\\'\\\"]\\s*(\\w+)[\\'\\\"]\\s*\\,?\\s*(\\[?[\\'\\\"\\w\\,\\s]*\\]?)\\s*\\)(\\s|\\n|\\t)*(\\-\\>(unsigned|unique|nullable|default)\\(\\)(\\;|\\n|\\t|\\s)*)?(\\-\\>(unsigned|unique|nullable|default)\\(\\)[\\;\\n\\t\\s]*)?#i", $input, $matches);
